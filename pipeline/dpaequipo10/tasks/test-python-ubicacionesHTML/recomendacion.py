...
@click.option('--inputfile1', type=click.Path())
@click.option('--inputfile2', type=click.Path())
@click.option('--outputfile', type=click.Path())
@click.option('--places', type=click.STRING)
@click.option('--radius', type=click.INT)
def main(bucket,inputfile1,inputfile2,outputfile,places,radius):
	...
	path = '/dpaequipo10/coordenadas.csv'
	f = open(path, 'wb')
	f.write(contenido)
	f.close()
	df=pd.read_csv(path)
	...
	k.key = inputfile2
	contenido = k.get_contents_as_string()
	path = '/dpaequipo10/types.csv'
	f = open(path, 'wb')
	f.write(contenido)
	f.close()
	types = pd.read_csv(path,header=None)

	#print("dataframes")
	#print("types: {}".format(types))
	#print("coordenadas {}".format(df))
	...
	for i in range(len(df)-1):
	    ...
	    for j in types:
	        url_places="https://maps.googleapis.com/maps/api/place/nearbysearch/json?location="+str(lat)+","+str(lon)+"&rankby=prominence"+"&radius="+str(radius)+"&type="+j+"&key="+places
	...
	idx=len(df)-1
	locations=locations+'{lat: '+str(df.loc[idx]['lat'])+', lng: '+str(df.loc[idx]['lon'])+'}]'
	if (df.loc[idx]['type']==0):
	        tipos=tipos+'"nivel1"];'
	        nombres=nombres+'"casa/trabajo/escuela"];'
	...
	k.key = outputfile
	k.set_contents_from_string(input_form)
	k.make_public()
